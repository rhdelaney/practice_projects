...
def print_results(results):
    print('%s\t%s\t%s\n' %
      (results['items'][0]['id'],results['items'][0]['snippet']['title'],results['items'][0]['statistics']['viewCount']))
...
def youtube_search(options):
  ...
  for search_result in search_response.get("items", []):
    if search_result["id"]["kind"] == "youtube#video":
      results=videos_list_by_id(service,part='snippet,statistics',id=search_result["id"])
      videos.append("%s\t%s\t%s\n" % (results["id"]["videoId"], results['snippet']['title'],
    results['statistics']['viewCount']))
      #videos_list_by_id(service,part='snippet,statistics',id=search_result["id"]["videoId"])
