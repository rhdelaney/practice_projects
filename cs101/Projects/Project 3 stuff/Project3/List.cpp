...
//CHECK THIS AFTER WATCHING ALL OF HIS VIDEOS TONIGHT AHHH AHHH AHHH AHHH
int List::size( ) {
	return n; } //n is the number of nodes in the list woot woot
...
void List::advance( ) {
	int num; 
	if(n==0 || current==NULL || current==tail){
		return;
	}
	else{
		num=current->rank;
...
int List::remove( ) { 
	...
	if(n==0 || current==NULL){
		...
		temp=current;
		current=temp->prev;
		//current->prev=temp->prev->prev->next;
