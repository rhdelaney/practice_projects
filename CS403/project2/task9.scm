  ...
  (define (recur big pow)
    (if (null? big)
      ...
      (+ (* (car big) (expt 10 pow))
            (recur (cdr big) (+ pow 1)))))
  (if (isNeg? big)
    (* -1 (recur (reverse (cdr big)) 0))
    (recur (reverse big) 0)))
(define (int-to-big int)
  (define (recur int)
    (if (= int 0)
      nil
      (cons (remainder int 10) (recur (/ int 10)))))
            ...
            (or (> (length operand1) (length operand2))
              (and (= (length operand1) (length operand2))
                (or (> (value operand1) (value operand2))
                  (and (= (value operand1) (value operand2)) (big> (pop operand1) (pop operand2)))
                )
              )
            )
          )
  ...
  (if (null? operand1)
    ...
    (if (null? operand2)
      ...
      (if (isNeg? operand1)
        ...
        (if (isNeg? operand2)
          #f
          (not (or (big> operand1 operand2) (big= operand1 operand2)))
        )
      )
    )
  )
...
(define (big+ augend addend)
  (define (add augend addend overflow)
    (if (and (null? augend) (null? addend))
      (if (= overflow 0) nil '(1))
      (begin
        (define sum (+ (value augend) (value addend) overflow))
        (append (list (remainder sum 10)) (add (pop augend) (pop addend) 
        (if (> sum 9) 1 0))
        )
      )
    )
...
(define (big- minuend subtrahend)
  ...
  (define (complement bigInt)
    ...
    (define (recur bigInt)
      (if (null? bigInt)
        nil
        (cons (- 9 (car bigInt)) (recur (cdr bigInt)))
      )
    )
  ...
  (define (add-zero bigInt n)
    (if (= n 0)
      bigInt
      (cons 0 (add-zero bigInt (- n 1)))
    )
  )
  (define length-diff
    (- (length minuend) (length subtrahend))
  )
  (cond
    ((and (null? minuend) (null? subtrahend)) '(0))
    ((null? minuend) (big- '(0) subtrahend))
    ((null? subtrahend) minuend)
    ((and (isNeg? minuend) (big< subtrahend minuend)) (big+ minuend (big-abs subtrahend)) )
    ((isNeg? minuend) 
      (begin 
        (define result (big+ (big-abs minuend) subtrahend))
        (if (isNeg? result) 
          result 
          (cons '- result)
        )
...
;         (begin
;           (define product (* (car multiplicand) multiplier))
;           (cons (+ (remainder product 10) overflow) (recur (cdr multiplicand) (/ product 10)))
;         )
;       )
;     )
...
;         (big+ (big+ (shift-right z2 (* 2 m2)) (shift-right (big- (big- z1 z2) z0) m2)) z0)
;       )
;     )
;   )
...
;         (reverse (int-to-big overflow))
;         (begin
;           (define product (* (car dividend) divisor))
;           (cons (+ (remainder product 10) overflow) (recur (cdr dividend) (/ product 10)))
;         )
;       )
;     )
