...
char *readTokenQ(FILE *fp){
    ...
    buffer = allocateMsg(size,"readToken");
    index = 0;
    while (!isspace(ch) && ch!='.' && ch!='(' && ch!=',' && ch!='[' && ch!=']' && ch!=')' && ch!='>' && ch!='<' && ch!='=' ){
        if (ch == EOF) break;
        if (index > size - 2){
            ++size;
            buffer = reallocateMsg(buffer,size,"readToken");
...
static char convertEscapedChar(int ch){
    switch (ch)
        {
        case 'n':  return '\n';
        case 't':  return '\t';
